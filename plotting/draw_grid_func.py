...
def locate_coord(cfgs: dict, subs: list[int], subc: list[int],
    ...
    Args:
        ...
        subs (list): Starting indices for x, y, z dimensions (0-based).
        subc (list): Count of points to select for x, y, z (-1 for all).
        subt (list): Stride for x, y, z dimensions.
...
def gather_coord(coordinfo: list, output_dir: str
    ...
    Returns:
        tuple: Three numpy arrays (x, y, z) containing the gathered coordinates.
    """
    max_i = 0
    max_j = 0
    max_k = 0
...
def gather_quality(coordinfo: list, output_dir: str, varnm: str
    ...
    """
    Gathers quality metric data from multiple NetCDF files.

    Args:
        coordinfo (list): List of dictionaries from locate_coord.
        output_dir (str): Directory containing the NetCDF files.
        varnm (str): Quality variable name.

    Returns:
        np.ndarray: Quality metric data.
    """
    max_i = 0
    max_j = 0
    max_k = 0
