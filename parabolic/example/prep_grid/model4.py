...
def main():
    ...
    if flag_topo_z:
        point_x = origin_x + (nx // 2) * dx  # MATLAB floor(nx/2)*dx
        point_y = origin_y + (ny // 2) * dy
        L = 400
        H = 2000
        for j in range(ny):
            for i in range(nx):
                r1 = (bz2[i, j, 0] - point_x)**2 + (bz2[i, j, 1] - point_y)**2
    ...
    # bz1 x: linspace(-500, 4500, 401), y: same, z=-2000
    x = np.linspace(-500, 4500, nx)
    y = np.linspace(-500, 4500, ny)
    for j in range(ny):
        for i in range(nx):
            bz1[i, j, 0] = x[i]
            bz1[i, j, 1] = y[j]
    ...
    if flag_printf:
        ...
        # MATLAB: plot3(bz2(201,:,1)/1e3,...) — cross-section at i=201
        mid_i = 201
        ax.plot(X[mid_i, :], Y[mid_i, :], Z[mid_i, :], 'r', linewidth=2)
        ...
        mappable.set_clim(-0.5, 0)
        cb = fig.colorbar(mappable, ax=ax, shrink=0.6, pad=0.1)
        cb.set_label('(km)', fontweight='bold')
        fig.patch.set_facecolor('white')
        plt.tight_layout()
        plt.savefig('../plot/para_model4.png', dpi=300, bbox_inches='tight', facecolor='white')
        plt.close(fig)
        print('Saved: ../plot/para_model4.png')
