  ...
  if (item = cJSON_GetObjectItem(root, "grid_method")) {
    ...
    if (subitem = cJSON_GetObjectItem(item, "elli_diri")) {
      ...
      if (thirditem = cJSON_GetObjectItem(subitem, "first_dire")) {
        ...
        if(strcmp(par->first_dire,"y") == 0)
        {
          par->first_dire_itype = Y_DIRE;
        }
      ...
      if (thirditem = cJSON_GetObjectItem(subitem, "second_dire")) {
        sprintf(par->second_dire, "%s", thirditem->valuestring);
        if(strcmp(par->second_dire,"x") == 0)
        {
          par->second_dire_itype = X_DIRE;
        }
        if(strcmp(par->second_dire,"y") == 0)
        {
          par->second_dire_itype = Y_DIRE;
        }
        if(strcmp(par->second_dire,"z") == 0)
        {
          par->second_dire_itype = Z_DIRE;
        }
      }
    }
    ...
    if (subitem = cJSON_GetObjectItem(item, "hyperbolic")) {
      ...
      if (thirditem = cJSON_GetObjectItem(subitem, "epsilon_x")) {
        par->epsilon_x = thirditem->valuedouble;
      }
      if (thirditem = cJSON_GetObjectItem(subitem, "bdry_x_type")) {
        par->bdry_x_itype = thirditem->valueint;
      }
      if (thirditem = cJSON_GetObjectItem(subitem, "epsilon_y")) {
        par->epsilon_y = thirditem->valuedouble;
      }
      if (thirditem = cJSON_GetObjectItem(subitem, "bdry_y_type")) {
        par->bdry_y_itype = thirditem->valueint;
      }
      if (thirditem = cJSON_GetObjectItem(subitem, "o2i")) {
        par->o2i = thirditem->valueint;
      }
...
{    
  ...
  if(par->method_itype == ELLI_DIRI) {
    ...
    if(par->first_dire_itype == Y_DIRE) {
      fprintf(stdout, "grid generate first direction is y\n");
    }
    if(par->first_dire_itype == Z_DIRE) {
      fprintf(stdout, "grid generate first direction is z\n");
    }
    if(par->second_dire_itype == X_DIRE) {
      fprintf(stdout, "grid generate second direction is x\n");
    }
    if(par->second_dire_itype == Y_DIRE) {
      fprintf(stdout, "grid generate second direction is y\n");
    }
    if(par->second_dire_itype == Z_DIRE) {
      fprintf(stdout, "grid generate second direction is z\n");
    }
  }
